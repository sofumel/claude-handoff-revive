...
2. **Resolve the action** based on `$ARGUMENTS`:
   ...
   - **`off`** — Disable auto-save for this session only:
     ...
     Confirm: "✓ Auto-save disabled for this session. Use `/handoff` to save manually whenever you want. New Claude sessions reset to enabled."
   ...
   - Anything else: tell the user usage is `/handoff-auto on | off | status`.
...
- Manual `/handoff` always works regardless of toggle.
